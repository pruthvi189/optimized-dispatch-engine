...
def test_fair_compare_identical_arrivals_prep_and_hub(tmp_path):
    ...
    # Pairing invariants: the same arrival times, prep durations, and hub
    # distances per order so both policies observed identical upstream events.
    # Under high demand (~422 orders/day) different dispatch timings cascade
    # into different rider availability → different cancellations, so we do
    # NOT enforce identical status across policies.
